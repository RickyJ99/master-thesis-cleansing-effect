...
\subsection{Introduction}
This thesis presents a model closely aligned with the theoretical structure discussed in \cite{OsePap17}, focusing
primarily on exploring how financial frictions impact the distribution of firms exiting the market in relation to their
productivity levels. Specifically, it investigates whether these frictions can explain the disparities observed between
previous economic downturns and the Great Recession. The novel aspects of this model include modifications in the flow
of funds and adjustments to the participation constraints of financial intermediaries, which collectively enhance the
tractability of the firms'  intertemporal maximization problem from a mathematical standpoint.

The subsequent sections delve into the revised formulation of the flow of funds and its dynamics. Initially, the
analysis tackles the maximization problem within a frictionless economic environment to establish a foundational
understanding. Following this, the focus shifts to scenarios where financial frictions are present, examining their
...
For a firm starting with initial capital \(k_0 = 0.2\) and debt \(b_0 = 0.1\), to maintain a steady state for both
capital and  debt, the dividends would be calculated as \(\widehat{d} = 0.5 \times 0.2^{0.8} - 0.1 \times 0.2 - 0.1
\times 0.1\). This specific combination of \(k_0 = 0.2\) and \(b_0 = 0.1\) represents a stationary point in the model.
...
Assuming the firm is debt-free for simplicity, the dynamics of fund flows adjust as follows:
\[
...
\]
thereby affecting the firm's capital accumulation and distribution strategy in response to changes in dividend payments.
\begin{figure}
    \centering
    \begin{tikzpicture}
        \begin{axis}[
...
Another way to visualize the participation constraint of the financial intermediaries is by the defining \(x=f(k)/b\).
\begin{figure}
    \centering
    \begin{tikzpicture}
        \begin{axis}[
...
In scenarios where dividends exceed their steady-state value, capital diminishes over time, suggesting a depletion of
resources contrary to situations where capital is below the steady state, leading to an accumulation of capital. This
dynamic illustrates the balancing act between leveraging debt for growth and the sustainable  management of capital and
dividends.
\begin{figure}
    \centering
    \begin{tikzpicture}
