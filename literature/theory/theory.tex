...
\[I = b_{t} + f(k_t) - d_t,\]
with \(d_t\) indicating the dividends disbursed at time \(t\).

The production function is:
\[f(k_t) = Z \cdot k_t^\alpha,\]
...
\[k_t = k_{t-1}(1 - \delta) - R \cdot b_{t-1} + b_{t} + f(k_t) - d_t,\]
where \(R = 1 + r\).
This expression can be simplified to:
\[k_t - k_{t-1}(1 - \delta) = f(k_t) - d_t + b_{t} - R \cdot b_{t-1},\]
leading to:
\[\Delta k_t = f(k_t) - \delta \cdot k_{t-1} - R \cdot b_{t-1} + b_{t} - d_t\]

Imposing a stationary condition for capital (\(\Delta k = 0\)):
\[0 = f(k_t) - \delta \cdot k_{t-1} - R \cdot b_{t-1} + b_{t} - d_t,\]
\[f(\widehat{k_t}) = \delta \cdot k + r \cdot b_{t-1} - \Delta b_{t} + d_t.\]
...
\[\Delta b_t = \Delta k_t - f(k_t) + \delta \cdot k_{t-1} + r \cdot b_{t-1} + d_t,\]
\[0 = \Delta k_t - f(k_t) + \delta \cdot k_{t-1} + r \cdot b_{t-1} + d_t,\]
\[\Delta k_t = f(k_t) - \delta \cdot k_{t-1} - r \cdot b_{t-1} - d_t.\]
...
\[f(\widehat{k}) = \delta \cdot \widehat{k} + r \cdot \widehat{b} + d_t.\]

It is demonstrable that when capital and debt stock are in equilibrium, dividends too reach a steady state:
\[d_t = f(\widehat{k}) - \delta \cdot \widehat{k} - r \cdot \widehat{b}.\]
...
\[\Delta k_t = f(k_t) - \delta \cdot k_{t-1} - d_t\]
\[k_{t+1} = f(k_t) + (1 -\delta)k_t - d_t \]
\begin{figure}
    \centering
    \begin{tikzpicture}
        \begin{axis}[
...
\[ b_{t} =  - f(k_t) + \delta \cdot \widehat{k} + R \cdot b_{t-1} + d_t\]
...
\begin{figure}
    ...
    blue line represents the flow of funds as modeled by the equation \(b_{t} =  - f(k_t) + \delta \cdot \widehat{k} + R
    \cdot b_{t-1} + d_t\). The red dot marks the threshold beyond which debt cannot exceed capital, effectively serving
    as a limit 
...
\[R=\frac{R_f}{p}  -\frac{ 1-p }{ p }\frac{f(k_t)}{b_t}\]
Lets draw a graph with the following paramenters \(p = 0.95, \delta = 0.1, \alpha = 0.8, Z = 0.5, d = 0.8,\widehat{k} =
3 \)
\begin{figure}
    \centering
    \begin{tikzpicture}
        \begin{axis}[
            ...
            ymin=0, ymax=2, % Set the y-axis limits
        ...
        {1.03/0.095-(0.05/0.095)*(0.5*3^0.8)/x -1};
    ...
    \end{tikzpicture}
    \caption{The graph illustrates the progression of debt under the condition that the change in capital (\(\Delta k\))
    is zero, with parameters set at \(\delta = 0.1\), \(r = 0.1\), \(\alpha = 0.8\), \(Z = 0.5\), and \(d = 0.8\), with \(\widehat{k} = 3\). The
    blue line represents the flow of funds as modeled by the equation \(b_{t} =  - f(k_t) + \delta \cdot \widehat{k} + R
    \cdot b_{t-1} + d_t\). The red dot, which should be at the intercept of the blue line with the \(R\) axis, marks the threshold beyond which debt cannot exceed capital, effectively serving
    as a limit on debt. The green dot signifies the steady state of the debt. The vertical or horizontal gap between the red and
    green dots quantifies the firm's equity. 
    }
\end{figure}
