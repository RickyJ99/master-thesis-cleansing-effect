...
starting capital that matches the minimum from the distribution of the capitals of exiting firms. 
...
parameter \(1-\mu = 0.25\). The aim is to discern the repercussions of such frictions on the economic dynamics. To this
...
both setups, firms face financial frictions manifested as monitoring costs, consistent with \(1-\mu=0.25\), in
accordance with the findings presented in \cite{OsePap17}. The ensuing figures present the sample mean and 95\%
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}
        ...
        \pgfplotstableread[col sep=comma, row sep=newline]{output_data/noexit_nofriction_stats.csv}\datatableNoExit
        ...
        \pgfplotstableread[col sep=comma, row sep=newline]{output_data/exit_nofriction_stats.csv}\datatableExit
...
\begin{figure}
    ...
    \begin{tikzpicture}
        ...
        \addplot+ [
            black, mark=*, mark options={fill=black},
            error bars/.cd,
        ...
        \addlegendentry{\(K_E - K_NE\) with CI}
        
        \end{axis}
    \end{tikzpicture}
    \caption{The graph depicts the log difference in total output \(K\) between two economies over a 20-step simulation
    with 100 iterations for 10 firms each. In one economy, underperforming firms are allowed to exit and their capital
    is reallocated, while in the other, there is no exit mechanism in place. Both economies experience financial
    frictions characterized by monitoring costs (\(1-\mu=0.25\)) and maintain fixed leverage. The data presented above
    illustrates a positive and statistically significant difference in overall output; the economy with an exit
    mechanism consistently demonstrates a higher level of total output.} 
