...
\begin{abstract}
    ...
    the firms' infinite intertemporal maximization dilemma, encapsulated within the participation constraints of
...
market frictions during economic downturns, particularly through policy interventions like demand stabilizers, might
influence the recession's 'cleansing effect'. This effect, akin to natural selection, is scrutinized for its potential
...
Building on established models like those of \cite{CabHarm94}, which discuss resource reallocation during
recessions, and extending the discussion to incorporate financial frictions as in \cite{OsePap17}'s work, this thesis
offers a unique perspective. Unlike previous studies that rely on numerical solutions, this research achieves a
methodological breakthrough by deriving a closed-form solution for the optimal paths of capital and dividends within the
Ramsey-Cass-Koopmans framework, providing a more precise understanding of firms' intertemporal maximization problems.

Monte Carlo simulations further explore the behavior of heterogeneous firms—varying in productivity and leverage—under
business cycle pressures. These simulations underscore the 'cleansing effect' of recessions, demonstrating a significant
efficiency boost in the economy's output in the absence of monitoring costs and a moderated effect when such costs are
introduced. This empirical evidence, aligning with insights from \cite{OsePap17}, confirms the theoretical propositions
...
\end{align}
Equation \autoref{eq24} presented implies that variation in capital stock depends on the balance between retained earnings and
...
The \autoref{fig:ph_d_k_nod_1} portrays the dynamics of dividends (\(d_t\)) in relation to the capital (\(k_t\)) of a firm, with a
...
\begin{figure}
    ...
    The blue arrows indicate a potential trajectory, known as a saddlepath, leading towards saddlepoint B. 
    }
...
Visual representation of total output over a 20-step horizon, including 100 iterations among a group of 10 firms,
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}
        \begin{axis}[
            title={Total Production: Actual vs. Optimal},
            xlabel={Step},
            xmin=1, ymin=0,
        ...
        \end{axis}
    \end{tikzpicture}
    \caption{Illustrating a 20-step simulation of 10 firms, this plot compares the chosen overall optimal output
    (\(K\)) (red line)
    against the
    overall actual capital (\(k\)) post-business cycle adjustment (blue line). The confidence intervals are set at 95\%.
    }
    \label{fig:optKnoE}
\end{figure}
...
Graph \autoref{fig:output_comparison_frictions} illustrates the effect of firm exits and capital reallocation on total output in two different economic
...
Figure \autoref{fig:output_comparison_frictions} affirms that capital reallocation following firm exits can enhance
...
application of Lagrangian methods to establish the steady state conditions. It progresses to leveraging the Bellman
...
productivity levels and leverage ratios. These simulations assess the impact of capital reallocation following firm
...
to increase its realism and applicability across different economic scenarios. Incorporating stochastic shocks to firm
