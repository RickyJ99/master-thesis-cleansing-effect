...
\begin{align}
    ...
    \text{for definition} \quad \blacksquare 
\end{align}
For transitivity, the order for \(\underline{k},\hat{k},\bar{k}\) is the following:
\begin{align*}
...
\(d_0\), which guarantees that the difference between production, and dividends is exactly equal to the replacemnt of
depreciated capital.
...
\begin{figure}
    ...
    quantity at which the system reaches a stationary state with zero dividends. The arrows illustrates the dynamics of dividends and capital in case of the firm finding itself outside the stable
    loci. Point A is a possible starting point (\(k_0,d_0\)). The arrows depict the evolution of dividends and capital
    when the firm is positioned away from the equilibrium points. Point A represents an initial position (\(k_0,d_0\)).
...
The sentence contains a few errors in grammar and clarity. Here's a corrected version:
...
Furthermore, in the steady state \(\hat{y}=f(\hat{k})\), the output is determined solely by \(Z\) due to its
construction, indicating that financial factors do not influence the relationship between the production function and
output. 
...
The participation constraint can be rewritten as follows:
\begin{align}
